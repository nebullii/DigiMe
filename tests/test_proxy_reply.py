from digime.cli import _extract_numbered_drafts
...
def test_build_proxy_draft_prompt_includes_retrieved_examples() -> None:
    prompt = build_proxy_draft_prompt(
        message="Can you review this today?",
        platform="discord",
        profile="discord",
        retrieved_examples="Example 1:\nIncoming context: Alex: can you review this?\nApproved reply: Yep, I can take a look later today.",
    )

    assert "Approved past replies in similar situations" in prompt
    assert "Approved reply: Yep, I can take a look later today." in prompt
...
def test_extract_numbered_drafts_handles_single_draft() -> None:
    formatted_package = """
New Discord message from Alex:
Can you review this?
...
Drafts:
1. Yep, I can take a look later today.
"""

    assert _extract_numbered_drafts(formatted_package) == [
        "Yep, I can take a look later today."
    ]
